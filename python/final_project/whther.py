...
def home():
    ...
    city = 'Netivot'
    country = 'Israel'
    whether_description = 'its good whether today'
    day_list = [['Sun', 20, 30, 4], ['Mon', 20, 30, 4], ['Tue', 20, 30, 4],
                ['Wed', 20, 30, 4], ['Thu', 20, 30, 4], ['Fri', 20, 30, 4],
                ['Sat', 20, 30, 4]]

    lat_lon = requests.get(
        "http://api.openweathermap.org/data/2.5/weather?q=" + city + "&units=metric&appid=96ad0ee0ffa1a71f9a327941c5e3c4b6")
    geo = json.loads(lat_lon.text)
    ...
    return render_template("index.html", day_list=dict, date_now=date_now, city=city,
                           country=country, whether_description=whether_description)
